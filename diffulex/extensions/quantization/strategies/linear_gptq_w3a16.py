"""
GPTQ W3A16 Linear Strategy

3-bit weight quantization with BF16 activation.
...
class GPTQW3A16LinearStrategy(LinearQuantizationStrategy):
    """
    GPTQ W3A16 linear quantization (3-bit).
    
    Uses vLLM's gptq_gemm op with bits=3.
    """
    ...
    @property
    def name(self) -> str:
        return f"gptq_w3a16_g{self.group_size}"
    ...
    def linear_forward(self, x: torch.Tensor, weight: torch.Tensor,
        ...
        # Fallback: manual dequantization (very slow for 3-bit)
        raise RuntimeError(
            f"[{self.name}] vllm.gptq_gemm kernel is required for 3-bit GPTQ. "
            f"Manual dequantization is not supported for 3-bit weights. "
